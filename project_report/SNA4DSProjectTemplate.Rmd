...
participants. [@dekker2007eurovision] has already
...
for valued networks [@dekker2005], which combine
...
In 2019, [@D_Angelo_Murphy_Alfò_2019] introduced the
...
In 2022 [@Ginsburgh_Moreno-Ternero_2022] suggested
...
we are going to run an Exponential Random Graph Model (ERGM) test. It offers
...
In the subsequent sections of the report, we'll explain the datasets we're
using and go through the steps of cleaning and preparing the data before any
analysis. We'll also provide a descriptive analysis to better understand the
data. Additionally, we'll discuss the reasons behind choosing the Conditional
Uniform Graph (CUG) and Exponential Random Graph Model (ERGM) methods for our
...
We can see in the table above that there are 2 terms with a very low
...
::: {style="display: flex;"}
![ProjectedNetworkMCMC1](images/ProjectedNetworkWithoutIsolatesMCMC1.png)
![Public Eurovision Network MCMC
2](images/ProjectedNetworkWithoutIsolatesMCMC2.png)
:::
...
![GOFMinGeodesicDistance](images/ProjectedNetworkWithoutIsolatesMinGeoDistance.png)
![GOFEdgeWiseSharedPartner](images/ProjectedNetworkWithoutIsolatesEdgeWiseSharedPartners.png)
:::
