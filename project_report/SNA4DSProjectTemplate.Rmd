...
-   H0: Governmental system and language family do not have an influence
    on how countries distribute their votes during Eurovision
    competition
...
the votes distribution is presented and used to create a network object.
...
following attributes for the nodes: - country_name -
country_language_family - country_government_system
...
this network. Because this network is undirected there are 4 types of
triad census present in the network 003, 102, 201 and 300. It does not
indicate a diverse and comprehensive set of structural configurations
among triplets of nodes in both networks.
...
Establishing a linkage between the focus of Exponential Random Graph Models (ERGMs) and the research query is pivotal. Harmonizing ERGMs with the inquiry ensures the measurement of pertinent network features, validating or refuting hypotheses based on modeled network structures. Based on the available data the second research question analyzes to
what extent language family or governmental political system influences
voting patterns during the Eurovision contest. These are both exogeneous
terms and their effect can be measured by ERGM models. This model allows
...
predictors, encapsulating structural properties as well as dyadic dependent terms. The observed network structure serves as the outcome variable, enabling estimation of tie probability based on these features to model network formation or evolution in social network analysis. 

1.  Baseline model (Claudia said it should be just with edges) edges are
    dyadic independent model (In ERGMs, the "edges" term models the
    presence or absence of edges between pairs of nodes (dyads) in the
    network. Dyadic independence implies that the presence or absence of
    an edge between any pair of nodes is considered independent of the
    presence or absence of edges in other dyads.)
    `r     ## Baseline ERGM model (prerequisities: must be a network object)     baseline_model_0.1 <- ergm::ergm(net_eurovision ~ edges)       (s1 <- summary(baseline_model_0.1))`

2.  Exploratory models (start introducing structural and dyadic
    dependent terms)
...
|-----------------------------------------|----------|------|------------|------|
| **edges**                               | Data     | Data | Data       | Data |
| **degree3**                             | Data     | Data | Data       | Data |
| **gwesp.fixed.0.001**                   | Data     | Data | Data       | Data |
| **nodematch.country_language_family**   | Data     | Data | Data       | Data |
| **nodematch.country_government_system** | Data     | Data | Data       | Data |
...
6.  Interpret results Interpretation: An increase of 1 in increases the
    log odds with (log odds) and it makes the odds of the effect taking
    place times larger. X1 θ1 exp( )θ1- 10 -
...
With the given model formula the following hypothesis were verified: -
H1: Governmental system and language family have an effect on how
countries distribute their votes during Eurovision competition
...
![Eurovision Public Centralities](images/PlotCentralities.png)
